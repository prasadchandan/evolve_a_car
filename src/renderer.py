...
class grLineWidth (pyglet.graphics.Group):
    """
    This pyglet rendering group sets a specific line size.
    """

    def __init__(self, size=4.0):
        super(grLineWidth, self).__init__()
        self.size = size
...
class PygletDraw(b2Draw):
    ...
    def DrawSegment(self, p1, p2, color):
        ...
        """
        self.batch.add(2, gl.GL_LINES, grLineWidth(3),
                       ('v2f', (p1[0], p1[1], p2[0], p2[1])),
                       ('c3f', [color.r, color.g, color.b] * 2))
...
class PygletWindow(pyglet.window.Window):
    ...
    def __init__(self, test, width, height):
        super(PygletWindow, self).__init__(width, height)
        self.test = test
...
class PygletFramework(FrameworkBase):
    ...
    def __init__(self):
        ...
        self.window = PygletWindow(self, fwSettings.screen_width, fwSettings.screen_height)
        ...
        self.scale_factor = self.computeScaleFactor()

    def computeScaleFactor(self):
        "Calculates the scale factor - to support HiDPI Screens"
        win_w, win_h = self.window.width, self.window.height
        fb_w, fb_h = self.window.get_framebuffer_size()
        return  int(max(float(fb_w) / win_w, float(fb_h) / win_h))
    ...
    def updateProjection(self):
        ...
        gl.glViewport(0, 0,
                      self.scale_factor * self.window.width,
                      self.scale_factor * self.window.height)
